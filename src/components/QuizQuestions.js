...
import {FecthQuestions} from "../store/questionsPage/QuestionActions";
...
import {
    ...
    selectQuestions,
    selectScore,
    selectCount, selectFinalCount,
    selectPreviousCategories, selectNewCategory
...
import {
    ...
    setScore,
    setFinalCount,
    setPreviousCategories, newCatNumber
...
export const QuizQuestions = () =>{
    ...
    if (finalCountDown >= 12) return <Navigate to="/"/>
    const round2 = ()=>{
        if(categoriesArray.includes(parseInt(newCategory))){
            ...
            dispatch(resetCount())
            dispatch(FecthQuestions(newCategory))
        }
    ...
    return(
        <>
            {count < 6 ? <Card>
                ...
                {questions? questions[qNumber]?.answers.map((ans,i)=>{
                    return(
                        <Button key={i} variant={"outline-secondary"} onClick={()=>{handleClick((ans.correct))}}>{ans.answer}</Button>
                    )
                }): " "}
            </Card>: <Button onClick={round2}>Round 2</Button>}
        </>
    )
}